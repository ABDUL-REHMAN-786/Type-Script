...
class CustomerInfo  {
    ...
    constructor(fullname:string, cnic:number,age:number,gender:string, accountNo: string,accountBal: number)
    {
...
let customers: { [cnic: number]: CustomerInfo} = {};

await writeWords( (chalk.greenBright.italic(`\n\t\t\tWelcome To AR-MY-BANK App`))); 
await writeWords( (chalk.greenBright(`\n\t\t\t**************************\n`))); 
...
if (selectOptions.option==="Create Account")
...

    {
        ...
        const Animation = chalkAnimation.rainbow(`\n\t\t\tThank you for using AR-MY-BANK App\n`);
        await new Promise(resolve => setTimeout
                            (resolve, 2000));
        Animation.stop();
    }
